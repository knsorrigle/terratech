  ...
  return (
    <div className="bg-[#050505] text-white font-sans selection:bg-amber-400 selection:text-black overflow-x-hidden min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center pt-20">
        ...
        {/* Cards + info strips — grouped so each strip stays under its card on mobile */}
        <div className="w-full flex flex-col md:flex-row max-w-[90vw] mx-auto border border-white/[0.06]">

          {/* LEFT GROUP — PROMPTHON */}
          <motion.div
            initial={{ opacity: 0, x: -60 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 1, ease: [0.16, 1, 0.3, 1], delay: 0.1 }}
            className="flex-1 flex flex-col border-b md:border-b-0 md:border-r border-white/[0.06]"
          >
            {/* Card */}
            <button
              onClick={() => navigate('/prompthon')}
              className="group relative flex-1 flex flex-col justify-between p-10 md:p-16 h-[60vw] md:h-[72vh] bg-[#050505] hover:bg-white transition-colors duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] text-left cursor-pointer overflow-hidden"
            >
            ...
            {/* Info strip */}
            <div className="flex items-center justify-between px-10 md:px-16 py-5 border-t border-white/[0.06]">
